...
class OpenMeteoWeatherIngestion:
    ...
    def __init__(self, username: str, dbutils, mode: str = "current") -> None:
        """
        ...
        self.mode = (
            "current" if mode == "current" else "hourly" if mode in ("forecast",'hourly') else None
        )
        self.target_folder: str = f"/Volumes/weather/open_meteo/open_meteo/{self.mode}/"
    ...
    def __write_data_to_folder(self, data: Dict[str, Any]) -> None:
        """
        ...
        json_payload: str = json.dumps(data, ensure_ascii=False, default=str)

        content_hash: str = hashlib.sha256(json_payload.encode("utf-8")).hexdigest()[
            :16
        ]
        ...
        file_name: str = f"{epoch_ns}_{self.mode}_{content_hash}_{unique_id}.json"
    ...
    def __process_weather_section(
        self, response_section: Any, base_results: Dict[str, Any]
    ) -> dict:
        """
        ...
        section_results.update(
            {
                variable_name: make_json_serializable(
                    variable_extraction(index).Value()
                )
                if self.mode.lower()  == "current"
                else make_json_serializable(variable_extraction(index).ValuesAsNumpy())
                for index, variable_name in enumerate(self.params[self.mode])
            }
        )

        if self.mode.lower() == "hourly":
            section_results.update(
                {"timestamp": make_json_serializable(self.get_times(response_section))}
            )
            section_results = [
        ...
        self.__write_data_to_folder(section_results)
    ...
    def get_weather(
        ...
        config_options: list[tuple[bool, Callable[[], None]]] = [
            (self.mode == "current", self.__current_config),
            (self.mode != "current", self.__forecast_16_hours_config),
        ]
        ...
        def get_results(response):
            ...
            sections: list[tuple[bool, Any, str, str]] = [
                (self.mode == "current", response.Current),
                (self.mode != "current", response.Hourly),
            ]

            for enabled, response_getter in sections:
                if enabled:
                    response_section = response_getter()

                    if response_section is not None:
                        self.__process_weather_section(
                            response_section, base_results)
